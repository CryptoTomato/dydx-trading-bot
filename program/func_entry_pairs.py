...
def open_positions(client):
    ...
    """
        ...
        Store trades for managing later on on exist position
    """

    # Load cointegrated pairs
    df = pd.read_csv("cointegrated_pairs.csv")

    # Get markets from referencing of main order size, tick size etc
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZCORE_THRESH:
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    # Ensure size is a factor of 10 for some coins where it's mandatory
